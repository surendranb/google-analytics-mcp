...
(Netlify), _headers + _routes.json (Cloudflare Pages), 404.html,
data/tools.json (11 tools from the package), data/skills.json.
...
# Static-only paths: served without invoking the Pages Function (Functions quota).
# Markdown twins stay in the include set because the Function reports their agent telemetry.
CF_ROUTES_EXCLUDE = ["/assets/*", "/favicon.svg", "/favicon-16.png", "/favicon-32.png",
                     "/apple-touch-icon-180.png", "/og.png", "/404.html"]
...
def build_cf_pages() -> None:
    """Cloudflare Pages config, generated like _redirects so site/dist/ stays the reviewed bytes.

    _headers uses Pages matching (a splat matches greedily across slashes, one per URL).
    The markdown rules cover direct raw fetches; negotiated responses get the same tags
    from the Function. _routes.json keeps static-only paths off the Functions quota.
    """
    headers = """# ga4mcp.com headers — Cloudflare Pages (generated by site/build.py)
#
...
# listed so no matcher edge case drops the tag on nested twins (/skills/<slug>/index.md).
/index.md
...
"""
    ...
    routes = {"version": 1, "include": ["/*"], "exclude": CF_ROUTES_EXCLUDE}
...
def parse_headers(text: str) -> dict[str, dict[str, str]]:
    """Parse a Pages _headers file: a path line, then indented Name: value lines."""
    ...
    for line in text.splitlines():
        ...
        if line[:1] not in (" ", "\t"):
            ...
            rules[path][name.strip()] = value.strip()
    return rules


def check_cf_pages() -> list[str]:
    """Shape-check the Cloudflare files so a bad deploy fails the build, not the preview."""
    problems: list[str] = []
    try:
        routes = json.loads((DIST / "_routes.json").read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"_routes.json is missing or invalid: {exc}"]
    if routes.get("version") != 1:
        problems.append(f"_routes.json version {routes.get('version')!r}, expected 1")
    if not routes.get("include"):
        problems.append("_routes.json needs at least one include rule")
    elif routes.get("include") != ["/*"]:
        problems.append(f"_routes.json include drifted: {routes.get('include')!r}")
    if routes.get("exclude") != CF_ROUTES_EXCLUDE:
        problems.append(f"_routes.json exclude drifted: {routes.get('exclude')!r}")
    rules = (routes.get("include") or []) + (routes.get("exclude") or [])
    if len(rules) > 100:
        problems.append(f"_routes.json has {len(rules)} rules (limit 100)")
    for rule in rules:
        if not rule.startswith("/") or len(rule) > 100:
            problems.append(f"_routes.json rule out of bounds: {rule!r}")

    try:
        headers_text = (DIST / "_headers").read_text(encoding="utf-8")
    except OSError as exc:
        return problems + [f"_headers is missing: {exc}"]
    expected = {
        "/*": ("Content-Signal", "ai-train=yes, search=yes, ai-input=yes"),
        "/index.md": ("X-Robots-Tag", "noindex"),
        "/*.md": ("X-Robots-Tag", "noindex"),
        "/*/index.md": ("X-Robots-Tag", "noindex"),
        "/assets/*": ("Cache-Control", "public, max-age=604800, immutable"),
    }
    rules_h = parse_headers(headers_text)
    for path, (name, value) in expected.items():
        got = rules_h.get(path, {}).get(name)
        if got != value:
            problems.append(f"_headers {path}: {name} is {got!r}, expected {value!r}")
    if len(rules_h) > 100:
        problems.append(f"_headers has {len(rules_h)} rules (limit 100)")
    if max((len(line) for line in headers_text.splitlines()), default=0) > 2000:
        problems.append("_headers has a line over the 2000-char limit")
    return problems
...
def main() -> int:
    ...
    for name in ("index.md", "llms.txt", "llms-full.txt", "robots.txt", "sitemap.xml", "_redirects", "_headers",
                 "_routes.json", "404.html",
